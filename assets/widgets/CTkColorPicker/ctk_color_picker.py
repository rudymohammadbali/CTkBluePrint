...
class AskColor(ctk.CTkToplevel):
    ...
    def create_color_widgets(self, color_mode):
        ...
        self.slider = ctk.CTkSlider(master=self.frame, border_width=self.slider_border,
                                    progress_color=color_mode, from_=0, to=255, button_length=8,
        ...
        self.label = ctk.CTkLabel(master=self.frame, text_color="#000000", height=40,
                                  fg_color=color_mode, text=color_mode)
        self.label.pack(fill="both", padx=10)

        self.button = ctk.CTkButton(master=self.frame, text=self.button_text, height=50, command=self._ok_event)
